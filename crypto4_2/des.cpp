...
void des_ofb_crypt(istream &src,uchar key[8],uchar c0_[8],ostream& dst){
    ...
    while(!src.eof()){
        ...
        src>>buf;
        print_bin(cout,buf);cout<<"->";
        print_bin(cout,buf^*c);cout<<endl;
        dst<<uchar(buf^*c);
        i++;i%=8;
        c++;
    }
...
void filemod(string filename,string mod){
